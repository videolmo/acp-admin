module XLSX
  class Billing < Base
    ...
    def build_worksheet(name)
      ...
      add_line("#{t('adjustments')}: #{ApplicationController.helpers.activity_human_name}", @memberships.sum(&:activity_participations_annual_price_change))
      ...
      add_line("#{t('amount')}: #{ApplicationController.helpers.activity_human_name}", @invoices.activity_participation_type.sum(:amount))
