...
describe ActivityParticipation do
  ...
  describe 'validations' do
    it 'validates halfday participants limit' do
      halfday = create(:activity, participants_limit: 3)
  ...
  describe '#validate!' do
    it 'sets states column and deliver activity_participations_validated email' do
      halfday = create(:activity, date: 3.days.ago)
      participation = create(:activity_participation, activity: halfday)
    ...
    it 'does not deliver activity_participations_validated email when timestamp was already set' do
      halfday = create(:activity, date: 3.days.ago)
      participation = create(:activity_participation, :validated, activity: halfday)
      expect { participation.validate!(admin) }
  ...
  describe '#reject!' do
    ...
    it 'does not deliver activity_participations_validated email when timestamp was already set' do
      halfday = create(:activity, date: 3.days.ago)
      participation = create(:activity_participation, :rejected, activity: halfday)
      expect { participation.reject!(admin) }
  ...
  describe '#destroyable?' do
    it 'always returns true by the default' do
      halfday = create(:activity, date: 2.days.from_now)
  ...
  describe '#send_reminder_email' do
    it 'sends an email when halfday is in less than two weeks' do
      halfday = create(:activity, date: 2.weeks.from_now - 1.hour)
    ...
    it 'does not send an email when halfday is in more than two weeks' do
      halfday = create(:activity, date: 15.days.from_now)
      participation = create(:activity_participation,
        activity: activity,
        created_at: 2.months.ago,
        latest_reminder_sent_at: nil)
    ...
    it 'sends an email when halfday is in less than three days' do
      halfday = create(:activity, date: 3.days.from_now - 1.hour)
      participation = create(:activity_participation,
        activity: activity,
        created_at: 2.months.ago,
